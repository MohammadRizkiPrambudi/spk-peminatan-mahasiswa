...
@section('content')
    <div class="content-wrapper">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">Tambah Relasi Matakuliah ke Bidang</h4>
                <form action="{{ route('admin.matakuliah-bidang.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <select name="matakuliah" id="matakuliah" class="form-control" required>
                            ...
                            @foreach ($matakuliah as $mk)
                                <option value="{{ $mk }}" {{ old('matakuliah') == $mk ? 'selected' : '' }}>
                                    {{ $mk }}</option>
                            @endforeach
                        </select>
                        @error('matakuliah')
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" step="0.01" min="0" max="1" name="bobot"
                            class="form-control @error('bobot') is-invalid @enderror" value="{{ old('bobot') }}" required>
                        @error('bobot')
